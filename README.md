...
python -m tester_framework --strategies orb_candle,orb_fvg,orb_combined --asset MNQ --timeframe 5m --risk_reward_ratio 2 --with_costs
```
...
```bash
python -m tester_framework --strategies orb_candle,orb_fvg,orb_combined --asset MNQ --timeframe 5m --time_period 5d --max_trades 3 --no_trade_html --workers 1
```
...
All three use 1-minute execution data (`EXECUTION_TIMEFRAME = "1m"`). For these strategies, `--timeframe` is the opening-range duration rather than the execution-bar size; every configured intraday duration from 1 minute through 1 hour is supported. They build the ORB high/low from the first NY session bars and look for breakouts during the 9:30–11:59 AM NY trade session. FVG detection runs in the 9:30–12:00 NY window with mitigation tracking. The SL offset uses the asset's configured tick size.
